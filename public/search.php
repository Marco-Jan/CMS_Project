...
$per_page = filter_input(INPUT_GET, 'per_page', FILTER_VALIDATE_INT) ?? 3; //$_GET['per_page];
...
if (!empty($search_term)) {
    $sql = 'SELECT COUNT(*) FROM articles WHERE published = 1 AND (title LIKE :search OR summary LIKE :search OR content LIKE :search)';
...
<main class="container mx-auto">
    <section class="flex flex-col justify-center items-center p-10">
        ...
        <form action="search.php" method="GET" class="mt-4">
            ...
            <input type="search" name="search" id="search" class="border p-2" value="<?php $search_term ?>">
            <button type="submit" class="bg-blue-500 text-white p-2">Search</button>
        </form>
    </section>
    <section class="flex flex-wrap p-8" id="content">
        <?php foreach ($articles as $article) : ?>
            <articles class="w-full p-4 flex justify-between flex-col sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6 mb-4">
                <a href="article.php?id=<?= $article['id'] ?>">
                    <img src="uploads/<?= e($article['image_file'] ?? 'blank.png') ?>" alt="<?= e($article['alttext']) ?>">
                ...
                <p class="credit text-xs">
                    Posted in <a class="text-pink-400" href="category.php?id=<?= e($article['category_id']) ?>">
                        <?= e($article['category']) ?></a>
                    by <a class="text-pink-400" href="user.php?id=<?= e($article['user_id'])  ?>">
                        <?= e($article['author']) ?></a>
                    </a>
                </p>
            </articles>
