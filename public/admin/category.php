...
if ($id) {
    ...
    if (!$category) {
        redirect('categories.php', ['error' => 'category no found']);
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {

    //daten werden aus dem Formular ausgelesen und vailidiert
    ...
    //daten werden auf vorhanden und länge validiert

    $errors['name'] = Validate::is_text($category['name'], 1, 50) && (!empty($category['name'])) ? '' : 'Name must be betweens 1 and 50 characters';
    ...
    //Fehler werden auf eine Zeichenkette zusammengefasst
    ...
    if (!$problems) {
        ...
        try {
            if ($id) {
                ...
                redirect('categories.php', ['successs' => 'category successfully saved']);
            }
...
<main class="container w-auto mx-auto md:w-1/2 flex justify-center flex-col items -center p-5">
    <form class="w-full grid" action="category.php?id=<?= $id ?? '' ?>" method="POST">
        ...
        <div class="p-4">
            <label class="block p-2.5 text-sm font-medium text-gray-900" for="description">Description</label>
            <textarea class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus_border-blue-500"
                name="description" id="description"><?= e($category['description']) ?></textarea>
