...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image_file'])) {
        ...
        if ($temp_path && $image['error'] === UPLOAD_ERR_OK) {
            // Alttext wird gesetzt

            $article['alttext'] = filter_input(INPUT_POST, 'alttext');
            // Validierung Alttext
            $errors['alttext'] = Validate::is_text($article['alttext'], 1, 254) ? '' : 'Alttext must be between 1 and 254 characters';
            // Bildtyp validierung
            $typ = mime_content_type($temp_path);
            $errors['filename'] .= in_array($typ, $allowed_types) ? '' : ' The filetype is not allowed';
            // Bildsendung wird validiert
            ...
            // Wenn keine Fehler, wird ein Speicherort für das Bild festgelegt
    ...
    $errors['content'] = Validate::is_text($article['content'], 1, 10000) ? '' : 'Content must be between 1 and 10000 characters';
    ...
    if (!$problems) {
        ...
        try {
            if ($save_to) {
                scale_and_copy($temp_path, $save_to);

                $cms->getArticle()->insertImage($imageBindings);

                $imageId = $cms->getArticle()->getLastInsertId();
                $bindings['images_id'] = $imageId;
            }

            if ($id) {
                $bindings['id'] = $id;
                $test = $cms->getArticle()->update($bindings);
                redirect('articles.php', ['success' => 'Article ' . $bindings['title'] . ' successfully changed']);
            } else {
...
<main class="p-10">
    ...
    <form action="article.php?id=<?= e($id) ?>" method="POST" enctype="multipart/form-data" class="grid gap-6 mb-6 md:grid-cols-2 md:w-full">
        ...
        <div>
            ...
            <select id="user_id" name="user" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                ...
                <?php foreach ($users as $user) : ?>
                    <option value="<?= $user['id'] ?>" <?= $user['id'] === $article['user_id'] ? 'selected' : '' ?>>
                        <?= e($user['forename']) ?> <?= e($user['surname']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
            ...
            <input type="text" id="alttext" name="alttext" value="<?= e($article['alttext'] ?? '') ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
