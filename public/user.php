...
// $articles = pdo_execute($pdo, $sql, ['id'=> $user_id])->fetchAll(PDO::FETCH_ASSOC);
...
<main class="container mx-auto mt-10 mb-10">
    ...
    <section class="felx flex-wrap p-8">
        <?php foreach($articles as $article) : ?>
            <article class="w-full p-4 flex justify-between flex-col sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6 mb-4">
                ...
                <p class="credit text-xs">
                    Posted in <a class="text-pink-400" href="category.php?id=<?= $article['category_id']?>"><?= e($article['category']) ?></a>
                    by <a class="text-pink-400" href="user.php?id=<?= $article['user_id'] ?>"><?= e($article['author']) ?></a>
                </p>
            </article>
            <?php endforeach; ?>
    </section>
